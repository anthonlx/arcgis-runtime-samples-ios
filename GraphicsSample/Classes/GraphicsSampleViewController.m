...
//--------------- city renderer --------------------------------------------------------------------------------
    ...
    AGSSimpleMarkerSymbol *censusMarkeySymbol = [AGSSimpleMarkerSymbol simpleMarkerSymbol];//默认是圆形
    censusMarkeySymbol.color = [UIColor greenColor];
	
    AGSSimpleMarkerSymbol *cityMarkerSymbol = [AGSSimpleMarkerSymbol simpleMarkerSymbol];
    cityMarkerSymbol.style = AGSSimpleMarkerSymbolStyleDiamond;//菱形
    cityMarkerSymbol.color = [UIColor brownColor];
    cityMarkerSymbol.outline.color = [UIColor redColor];

    AGSSimpleMarkerSymbol *townMarkerSymbol = [AGSSimpleMarkerSymbol simpleMarkerSymbol];
    townMarkerSymbol.style = AGSSimpleMarkerSymbolStyleCross;//十字架
    townMarkerSymbol.outline.color = [UIColor blueColor];
    townMarkerSymbol.outline.width = 3.0;

    cityRenderer.uniqueValues = @[
                                 [AGSUniqueValue uniqueValueWithValue:@"census designated place"
                                                               symbol:censusMarkeySymbol],
	...
	//apply city renderer
    self.cityGraphicsLayer.renderer = cityRenderer;//修改city的渲染器
//--------------- city renderer --------------------------------------------------------------------------------
...
//TODO: 查询 county 和 city 的 Graphic
...
- (void)queryTask:(AGSQueryTask *)queryTask operation:(NSOperation *)op didExecuteWithFeatureSetResult:(AGSFeatureSet *)featureSet {
	...
	if (YES == [featureSet.displayFieldName isEqualToString:@"CITY_NAME"]) {
        ...
        fillSymbol.outline.color = [UIColor redColor];//红色线边框
